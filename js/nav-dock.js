/* ============================================================
   ...
   "Unlock Your Zodi Animal" CTA; after a reveal it shows the
   animal with a share icon. Tapping the animal opens a short
   action sheet (read / compare / reveal another); tapping share
   opens a share modal with 12 platforms, GA attribution, and a
   copy-link fallback.

   Pure progressive enhancement, mobile only (styling in
   nav-dock.css scopes it to <=900px). Reads the same profile
   nav-me.js uses (zodi:home-v2:profile) so it needs no engine.
   Never throws.
   ============================================================ */
...
PNAV.features.dock = function (ctx) {
  "use strict";
  try {
    if (document.getElementById("pn-dock")) return; /* once per page */
    ...
    var revealed = !!(prof && name);

    /* ---- share text + attributed url ---------------------------------- */
      ...
      try {
        if (typeof window.gtag === "function") {
          window.gtag("event", "share", {
            method: method, content_type: "zodi_animal", item_id: SHARE_SLUG
          });
        }
      } catch (e) {}
    }
    ...
    function cdn(slug) { return '<img src="https://cdn.simpleicons.org/' + slug + '/d6c18c" alt="" aria-hidden="true">'; }

    /* platforms in the approved order */
    var PLATFORMS = [
    ...
    if (!revealed) {
      ...
      sheet.innerHTML =
        ...
        '<a class="pn-dock-act" href="/index.html#read">' +
    ...
    function buildModal() {
      ...
      modal.addEventListener("click", function (e) {
        ...
        if (t) {
          ...
          var href = INTENTS[pid] ? INTENTS[pid](url) : url;
          window.open(href, "_blank", "noopener");
          return;
        }
        if (e.target.closest(".pn-dock-modal-close") || e.target === modal) closeShareModal();
      });
      document.addEventListener("keydown", function (e) {
    ...
    /* native share sheet on the animal glyph long-press? kept simple: no. */
